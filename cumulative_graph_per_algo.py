...
def get_cumulative_data_per_algo(grouped_data, ATTR):
    ...
    for domain, problems in grouped_data.items():
        for problem, algos in problems.items():
            for algo, val_list in algos.items():
                if algo not in temp_data:
                    temp_data[algo] = {'unsorted':[]}
                unsorted = []
                for val in val_list:
                    if ATTR in val:
                        unsorted.append(val[ATTR])
                temp_data[algo]['unsorted'] += unsorted
    cumu_data = {}
    for algo, data in temp_data.items():
        unsorted = data['unsorted']
        if len(unsorted) > 0:
            sorted_data = sorted(unsorted)
            cumu = 1
            cumu_data[algo] = {'x': [sorted_data[0]], 'y': [cumu]}
            for x in sorted_data:
                if cumu_data[algo]['x'][-1] == x:
                    cumu_data[algo]['y'][-1] = cumu
                else:
                    cumu_data[algo]['x'].append(x)
                    cumu_data[algo]['y'].append(cumu)
                cumu += 1
...
def get_plot_data_from_cumu(cumu_data, max_y):
    graph_data = {}
    for algo, xy_data in cumu_data.items():
        x = xy_data['x']
        y = xy_data['y']
        if algo == 'base_unsat':
            y[len(y) - 1] = max_y
        graph_data[algo] = {'x': x, 'y': y}
    return graph_data
...
def create_cumulative_graph_from_plot_data(graph_data, OUTDIR, ATTR, xlog_scale):
    ...
    subplt.set_xlabel(ATTR)
    subplt.set_ylabel('Cumulative count')
    for algo, xy_data in graph_data.items():
        if algo == 'base_unsat':
        ...
        if xlog_scale:
            ...
            if len(str(xy_data['x'][0])) > 5:
                plt.setp(subplt.get_xticklabels(), rotation=30, horizontalalignment='right')
        subplt.margins(0.05)
    plt.legend(handler_map={subplt: HandlerLine2D(numpoints=1)}, prop=fontP, loc='best')
    plt.savefig(OUTDIR + '/cumu.png')
    plt.close()
    print('Done')
    return
...
def main():
    ...
    if check_attribute_exists(data, args.attribute):
        data, max_y = get_cumulative_data_per_algo(data, args.attribute)
